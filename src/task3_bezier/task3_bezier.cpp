// Task 3 - Render the letter R using OpenGL
...
private:
    ...
    std::vector<glm::vec2> controlPoints2 = {
            {0.13, 0.22},
            {0.385, -0.95},
            {-0.385, -0.95},
            {-0.13, 0.22}
    ...
    glm::vec2 interpolate(const glm::vec2 &p0, const glm::vec2 &p1, const float t){
    ...
    glm::vec2 bezierPoint(const glm::vec2 &p0, const glm::vec2 &p1, const glm::vec2 &p2, const glm::vec2 &p3, const float t) {
        ...
        glm::vec2 linear_p1 = interpolate(p0, p1, t);
        glm::vec2 linear_p2 = interpolate(p1, p2, t);
        glm::vec2 linear_p3 = interpolate(p2, p3, t);

        glm::vec2 quadratic_p1 = interpolate(linear_p1, linear_p2, t);
        glm::vec2 quadratic_p2 = interpolate(linear_p2, linear_p3, t);

        glm::vec2 cubic_p1 = interpolate(quadratic_p1, quadratic_p2, t);
        return cubic_p1;

        /* Calculation by equation
        float cubic_bezier_x = (powf((1-t),3) * p0.x) + (3 * powf((1-t), 2) * t * p1.x) + (3 * (1 - t) * powf(t,2) * p2.x) + (powf(t,3) * p3.x);
        float cubic_bezier_y = (powf((1-t),3) * p0.y) + (3 * powf((1-t), 2) * t * p1.y) + (3 * (1 - t) * powf(t,2) * p2.y) + (powf(t,3) * p3.y);

        return {cubic_bezier_x, cubic_bezier_y};
         */
    }
    ...
    void bezierShape(int count) {
        for(int i = 1; i < (int) controlPoints.size(); i+=3) {
            for (int j = 0; j <= count; j++) {
                ...
                glm::vec2 point;
                float t = 1 / float(count) * float(j);
                point = bezierPoint(controlPoints[i - 1], controlPoints[i], controlPoints[i + 1], controlPoints[i + 2], t);
                points.emplace_back(point, 0);
            }
        }
        // For the foot of mushroom
        for(int i = 1; i < (int) controlPoints2.size(); i+=3) {
            for (int j = 0; j <= count; j++) {
                // TODO: Generate points for each Bezier curve and insert them
                glm::vec2 point; //= ??
                float t = 1 / float(count) * float(j);
                point = bezierPoint(controlPoints2[i - 1], controlPoints2[i], controlPoints2[i + 1], controlPoints2[i + 2], t);
                points.emplace_back(point, 0);
            }
        }
    }
...
public:
    BezierWindow() : Window{"task3_bezier", SIZE, SIZE} {
        ...
        program.setUniform("ModelMatrix", glm::mat4{1.0});
        program.setUniform("ViewMatrix", glm::mat4{1.0});
        program.setUniform("ProjectionMatrix", glm::mat4{1.0});
